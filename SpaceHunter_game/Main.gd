...
func _on_CrystalTimer_timeout():
	...
	# í ½í´½ SAFE SPAWN (works with circle OR rectangle shapes)

	var cs = drop.get_node("CollisionShape2D")
	var shape = cs.shape

	var half_w = 16.0  # fallback default
...
func _on_Catcher_body_entered(body):
	...
	# If it's a "good" drop (positive points), missing it costs a life.
	...
	if body.has_method("get") and body.get("points") != null:
		...
		# Fallback: if something without "points" hits the catcher, treat as good
		life -= 1
		$Menu/Life.text = "Life: " + str(life)
